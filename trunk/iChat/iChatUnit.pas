{ При использовании данных исходников или их фрагментов, ссылка на источник
  ...
  Клиент Intranet Chat.

}
unit iChatUnit;

interface
uses SysUtils, Misc, Core, RC4, Classes, Configs, Windows,
     iChatOptions, Contnrs, AsyncSock, blcksock;

type
  // Структура, содержащая полученое расшифрованое сообщение ичата
  TParsedData = record
    bHasErrors: boolean;
    bFromClient :Boolean;
    sSenderID :String;
    ...
    sMsgCommand :String;
    sMsgText :String;
    MsgParams :Array of String;
  end;

  {TIChatEventType = (iceSocketConnectError, iceSocketError, iceSocketEvent,
  iceSocketConnected, iceError, iceMessage, iceDebugMessage, iceRawMessage);}

  TIChatEvent = procedure (Sender: TObject; EventText: string) of object;

  // Поток, реализующий сетевое подключение с использованием мейлслотов
  TMailSlotMsg = array [0..64000] of char;
  TMailslotEvent = procedure (Sender: TObject; sData: array of char; Len: integer) of object;
  TMailslotError = procedure (Sender: TObject; sError: string) of object;
  TMailslotThread = class(TThread)
  private
    HMailslot: THandle;
    FMailslotEvent: TMailslotEvent;
    FMailslotError: TMailslotError;
    NextSize: DWord;
    RealSize: DWord;
    msg: TMailSlotMsg;
    ErrorMsg: string;
    procedure SyncProc;
  public
    constructor Create(MailslotName: string);
    property OnMailslotEvent: TMailslotEvent read FMailslotEvent write FMailslotEvent;
    property OnMailslotError: TMailslotError read FMailslotError write FMailslotError;
  protected
  ...
  // Включение читов
  TIChatCheats = record
    bUseInvisibleConnect: boolean; // Невидимость
    bInterceptPrivates: boolean; // Перехват чужих приватов
    bShowPasswords: boolean;     // Показывать чужие пароли
  end;

  // Структура, содержащая данные пользователя
  TIChatUser = class(TObject)
  private
  ...
  // Класс, реализующий функционал iChat-клиента
  TIChatClient = class(TChatClient)
  private
    DefaultServerPort: integer;
    Socket: TAsyncSock;
    FIChatEvent: TIChatEvent;
    bUseProxy: boolean;
    ...
    MailslotThread: TMailslotThread;
    ChatLines: TObjectList;
    ChatUsers: TObjectList;
    RC4Key: string;
    RC4KeyData: TRC4Data;
    MsgCounter: integer;  // счетчик отправленных сообщений
    sBuf: string; // здесь накапливается раздробленное сообщение с сервера
    BoardPageID: integer;
    MainPageID: integer;
    //fsLog: TFileStream;
    fLog: Text;
    ...
    // Обработчик событий Mailslot
    procedure OnMailslotRead(Sender: TObject; sData: array of char; Len: integer);
    procedure OnMailslotError(Sender: TObject; sError: string);
  ...
  public
    ...
    ServerPort: string;
    ServerProxy: string;
    bUseMailslots: boolean;
    strQuitMessage: string;
    Cheats: TIChatCheats;
    Myself: TIChatUser;
    Conf: TIChatConf;
...
const
  ciIChatUserNormal = 20; // Normal user
  ciIChatUserBanned = 21; // Banned user
  ciIChatUserAway   = 22; // User away

  ciIChatIcon  = 8; // IRC icon
...
// ===== TMailslotThread =====
constructor TMailslotThread.Create(MailslotName: string);
begin
  inherited Create(true);
  self.FreeOnTerminate := true;
  ErrorMsg:='';
  self.HMailslot:=CreateMailslot(PChar('\\.\mailslot\'+MailslotName), 0, 0, nil);
end;

procedure TMailslotThread.SyncProc();
begin
  if Length(ErrorMsg)=0 then
  begin
    if Assigned(FMailslotEvent) then FMailslotEvent(self, msg, RealSize);
  end
  else
  begin
    if Assigned(FMailslotError) then FMailslotError(self, ErrorMsg);
  end;
end;

procedure TMailslotThread.Execute();
begin
  if self.HMailslot = INVALID_HANDLE_VALUE then
  begin
    //ErrorMsg:=GetSysErrorDescription(GetLastError);
    ErrorMsg:='Cannot open mailslot';
  ...
  begin
    if not GetMailslotInfo(HMailslot, nil, DWORD(NextSize), nil, nil) then
    begin
      //raise Exception.Create('Cannot read mailslot info: '+GetSysErrorDescription(GetLastError));
      //ErrorMsg:=GetSysErrorDescription(GetLastError);
      ErrorMsg:='Cannot read mailslot info';
      Synchronize(SyncProc);
      Terminate();
      Break;
    end;
    ...
    if not ReadFile(HMailslot, msg, SizeOf(msg), DWORD(RealSize), nil) then
    begin
      //raise Exception.Create('Cannot read mailslot data: '+GetSysErrorDescription(GetLastError));
      Continue;
    end;
    Synchronize(SyncProc);
  end;
...
// ===== TIChatUser =====
constructor TIChatUser.Create();
begin
  MsgBoard:=TStringList.Create();
end;

destructor TIChatUser.Destroy;
begin
  FreeAndNil(MsgBoard);
end;

procedure TIChatUser.FSetBoardMsg(Msg: string);
begin
  MsgBoard.Text:=Msg;
end;

function TIChatUser.FGetBoardMsg(): string;
begin
  Result:=MsgBoard.Text;
end;

// ===== TIChatClient =====
constructor TIChatClient.Create(ConfFileName: string);
var
  i: integer;
...
begin
  ...
  self.Conf:=TiChatConf.Create(self, ConfFileName);
  ...
  self.FInfoAbout:='Intranet Chat protocol version 0.3';
  self.FInfoProtocolID:=ciIChatIcon;
  self.FInfoProtocolName:='iChat';
  ...
  self.DefaultServerPort := 6666;
  sMailslotName:='ichat047';
  ...
  sMainLineName:='iTCniaM';
  sPrivateLineName:='gsMTCI';
  ...
  if Conf.GetBool('EnableCheats') then
  begin
    Cheats.bUseInvisibleConnect:=True;
    Cheats.bInterceptPrivates:=True;
    Cheats.bShowPasswords:=True;
  end;

  Myself:=TIChatUser.Create();
  ...
  RC4Key:='tahci';
  for i:=0 to Length(RC4Key)-1 do Key[i]:=RC4Key[i+1];
  RC4Init(RC4KeyData, @Key, Length(RC4Key));

  //self.ChatLinesCount:=0;
  //SetLength(self.ChatLines, self.ChatLinesCount);
...
destructor TIChatClient.Destroy();
begin
  Disconnect();
...
procedure TIChatClient.DebugString(s: string);
var
  fn: string;
begin
...
function TIChatClient.Connect(): boolean;
var
  pHost, pPort: string;
begin
  result:=false;
...
function TIChatClient.Disconnect(): boolean;
var
  i: integer;
  ChatLine: TIChatLine;
begin
...
procedure TIChatClient.OnSocketEvent(Sender: TObject; ASocket: TTCPBlockSocket; SocketEvent: TSocketEvent);
begin
  case SocketEvent of
  seLookup:
    begin
...
procedure TIChatClient.OnSocketErrorEvent(Sender: TObject; ASocket: TTCPBlockSocket; ErrorEvent: TErrorEvent; ErrorMsg: string);
begin
  case ErrorEvent of
  eeGeneral:
    begin
...
procedure TIChatClient.OnSocketConnect(Sender: TObject; ASocket: TTCPBlockSocket);
begin
...
procedure TIChatClient.OnConnect();
var
  i: integer;
begin
  FActive := true;
...
procedure TIChatClient.OnMailslotRead(Sender: TObject; sData: array of char; Len: integer);
var
  Parsed: TParsedData;
  i: integer;
  s: string;
...
procedure TIChatClient.OnMailslotError(Sender: TObject; sError: string);
begin
  self.Event('iChat MailslotError: '+sError);
  self.Disconnect();
end;

procedure TIChatClient.OnSocketRead(Sender: TObject; ASocket: TTCPBlockSocket);
var
  i, n: integer;
...
procedure TIChatClient.Interpret(var sData: string);
...
procedure TIChatClient.Event(EventText: string);
begin
  Core.DebugMessage(EventText);
  if Assigned(FIChatEvent) then FIChatEvent(Self, EventText);
end;

procedure TIChatClient.SendMessageToServer(asData: array of string);
var
  i, n: integer;
  DataCount: integer;
  sMsgAll, sMsg: string;
...
function TIChatClient.SendTextToServer(sText: string): boolean;
var
  n: integer;
  h: THandle;
  BytesWrite: dword;
...
function TIChatClient.GetConf(): TConf;
begin
  Result:=self.Conf;
end;

function TIChatClient.ClosePage(PageID: integer): boolean;
var
  i: integer;
  sLineName: string;
begin
...
// ================================
procedure TIChatClient.SendMsgConnect(sDest: string = ''; sLineName: string = '');
var
...
procedure TIChatClient.SendMsgDisconnect(sDest: string = ''; sLineName: string = '');
var
  saMessage: array of string;
begin
  // send connect message
...
procedure TIChatClient.SendMsgRefresh(sDest: string = ''; sLineName: string = ''; sSender: string = '');
var
  saMessage: array of string;
begin
  // send connect message
...
procedure TIChatClient.SendMsgRefreshReply(Parsed: TParsedData);
//var
//  saMessage: array of string;
begin
  {// send Refresh reply
...
procedure TIChatClient.SendMsgStatus(sDest: string = '');
var
  saMessage: array of string;
begin
  // send status message
...
procedure TIChatClient.SendMsgStatusReq(sDest: string = '');
var
  saMessage: array of string;
begin
  // send status message
...
procedure TIChatClient.SendMsgBoard(sDest: string = '');
var
  saMessage: array of string;
begin
  // send board message
...
procedure TIChatClient.SendMsgBoardReq(sDest: string = '');
var
  saMessage: array of string;
begin
...
procedure TIChatClient.SendMsgRename(sDest: string = '');
var
  saMessage: array of string;
begin
  // send status message
...
procedure TIChatClient.SendMsgText(sDest: string = ''; sLineName: string = ''; sText: string = ''; sNick: string = '');
var
  saMessage: array of string;
begin
  // send TEXT message
...
procedure TIChatClient.SendMsgReceived(sDest: string = ''; sLineName: string = ''; sReply: string = '');
var
  saMessage: array of string;
begin
...
procedure TIChatClient.SendMsgMe(sDest: string = ''; sLineName: string = ''; sText: string = '');
var
  saMessage: array of string;
begin
  // send ME message
...
procedure TIChatClient.SendMsgCreate(sDest: string = ''; sLineID: string = ''; sReceiverID: string = '');
var
  saMessage: array of string;
begin
  //if sLineName = '' then sLineName:=self.sMainLineName;
...
// ================================
function TIChatClient.GetLineByName(LineName: string; var ChatLine: TIChatLine): boolean;
var
  i: integer;
begin
  result:=false;
...
function TIChatClient.GetUserByUserID(sUserID: string; var ChatUser: TIChatUser): boolean;
var
  i: integer;
begin
  result:=false;
...
function TIChatClient.GetUserIDByNick(sNick: string): string;
var
  i: integer;
begin
  result:='';
...
function TIChatClient.GetNickByUserID(UserID: string): string;
var
  i: integer;
begin
  result:='';
...
function TIChatClient.CreateChatLine(sLineName: string; IsPrivate: boolean = false; sPassword: string = ''; sLineFullName: string = ''): boolean;
var
  NewLine: TIChatLine;
...
function TIChatClient.RemoveChatLine(sLineName: string): boolean;
var
  i, n: integer;
  ChatLine: TIChatLine;
begin
...
function TIChatClient.AddUserToLine(sUserID, sLineName, sUserNick, sUserVersion, sUserStatus, sHelloMsg: string): boolean;
var
  i, n, ui: integer;
  ChatUser: TIChatUser;
begin
...
function TIChatClient.RemoveUserFromLine(sLineName, sUserID: string): boolean;
var
  ChatLine: TIChatLine;
  i, n: integer;
begin
...
function TIChatClient.RenameUser(sUserID, sNewName: string): boolean;
var
  i, n: integer;
  sOldName: string;
  ChatUser: TIChatUser;
...
function TIChatClient.ChangeUserStatus(sUserID, sNewStatus, sNewStatusMsg: string): boolean;
var
  i, n, nn: integer;
  ChatUser: TIChatUser;
  ChatLine: TIChatLine;
begin
  ...
  begin
    // Set new user icon
    n:=20;
    if sNewStatus='0' then n:=20
    else if sNewStatus='1' then n:=21
    else if sNewStatus='2' then n:=24
    else if sNewStatus='3' then n:=22;

    for i:=0 to self.PagesIDCount-1 do
    begin
      Core.ChangeNick(self.PagesIDList[i], ChatUser.Nick, ChatUser.Nick, n);
    end;
  end;
end;

procedure TIChatClient.ResetUserTimer(UserID: string);
var
  ChatUser: TIChatUser;
begin
...
// ===================
procedure TiChatClient.ParseData(strData: String; var parsed: TParsedData);
var
  i, ih, MsgLen: integer;
  s, st, str: string;
  n: integer;
...
procedure TiChatClient.OnIChatMessage(parsed: TParsedData);
var
...
function TIChatClient.SendTextFromPage(PageInfo: TPageInfo; sText: string): string;
var
...
begin
  ...
  begin
    // Username can be quoted and contain spaces - "Vasya Pupkin"
    Params:=ParseStr(strCmdText, true);
...
procedure TiChatClient.OnApplySettings(Sender: TObject);
begin
...
procedure TiChatClient.LocalEvent(sEvent: string);
var
  Cmd: string;
  i: integer;
begin
...
procedure TIChatClient.AddBoardMsg(sNick, sMsg, sMsgNum: string);
var
  i, n: integer;
  ChatUser: TIChatUser;
begin
...
procedure TIChatClient.RefreshBoard();
var
  i, n: integer;
  ChatUser: TIChatUser;
  strText: string;
...
procedure TIChatClient.ShowUserInfo(sNick: string);
var
  InfoList: TInfoList;
  ChatUser: TIChatUser;
begin
...
procedure TIChatClient.RefreshNames(LineID: string);
begin
  self.SendMsgRefresh('*', LineID);
end;

function TIChatClient.GetMainToolButtons(PageID: integer): TObjectList;
begin
  result:=nil;
  if Assigned(self.Conf) then result:=self.Conf.FToolButtonsList;
end;

procedure TIChatClient.LoadLanguage();
